...
- Le nom de dossier, la référence et la date sont modifiables et conservés dans les brouillons et les rapports.
- Dossiers (26 septembre) : créés dans « Mes dossiers » ou à l'étape 1 d'un constat, stockés dans le `localStorage` de l'appareil (`dossiers.js`). Chaque constat mémorise son `dossierId` ; la sauvegarde JSON d'une visite contient une copie du dossier, recréé à l'import sur un autre appareil. Supprimer un dossier conserve ses constats dans « Mes brouillons ». Le tableau de bord ne contient plus de données fictives : chiffres, dossiers récents et échéances proviennent des dossiers et brouillons de l'appareil.
