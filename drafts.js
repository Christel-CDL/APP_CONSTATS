...
async function startNewVisit(dossierId){
  ...
  try{if(state.photos.length||state.caseName||state.subjects.length>1||collectActions().some(a=>a.text||a.recipient)){if(!(await persistVisit()))return;}
    suspendVisitAccess();clearTimeout(saveTimer);state.draftId=createId();state.photos=[];state.subjects=['Vue générale'];state.activeSubject='Vue générale';state.position=null;state.dossierId=null;state.caseName='';state.caseReference='';state.visitDate=new Date().toLocaleDateString('en-CA');if(typeof dossierId==='string'&&dossierById(dossierId)){const dossier=dossierById(dossierId);state.dossierId=dossier.id;state.caseName=dossier.name;state.caseReference=dossier.reference;touchDossier(dossier.id);}syncDossierPicker();renderCaseDetails();$('#action-list').innerHTML='';addAction();selectCase(0);state.currentStep=1;renderSubjects();renderCaptures();showPosition();showPage('inspection');renderStep();saveVisit();prepareVisitAccess();
...
async function openDraft(draftId,button){
  ...
  try{if(unsaved&&!(await persistVisit()))return;const drafts=await listDrafts(),draft=drafts.find(d=>d.draftId===draftId);if(!draft)return;suspendVisitAccess();restoreState(draft);state.currentStep=2;showPage('inspection');renderStep();saveVisit();}
